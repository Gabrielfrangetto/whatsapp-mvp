...
export const TREND_H = 110;
export const TH_STYLE = { padding: '8px 10px', textAlign: 'center', fontSize: 10, fontWeight: 700, color: 'var(--theme-text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em', borderBottom: '1px solid var(--theme-border)', borderRight: '1px solid var(--theme-border)', background: 'var(--theme-bg-tertiary)', whiteSpace: 'nowrap' };
...
export function PeakMini({ peakHours }) {
  const [hovered, setHovered] = useState(null);
...
export function TrendTable({ dailyTrend }) {
...
export function PeakTable({ peakHours }) {
  const max = Math.max(...peakHours, 1);
  return (
    <div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid var(--theme-border)' }}>
      <table style={{ borderCollapse: 'collapse' }}>
...
export function AgentBar({ agent, count, max }) {
...
export function WorkloadTable({ agents, totalChats }) {
  return (
